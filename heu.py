...
def split_registros_con_fusion(texto, campos_por_registro):
    ...
    while i + campos_por_registro - 1 < len(campos):
        ...
        for campo in registro:
            ...
            print(f'[DEBUG] Campo {x} : {campo} : {len(campo)} caracteres')
            x += 1
        total = sum(len(reg) for reg in registro)
        print(f'[DEBUG] Longitud total del registro: {total}')
        print(
            f'[DEBUG] Longitud de registros + separadores: {total + len(registro)} - 1 si no hay separador final {total + len(registro) - 1}')
        break
